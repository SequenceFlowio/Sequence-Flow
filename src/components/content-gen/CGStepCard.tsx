...
interface CGStepCardProps {
  step: StepData;
  cardRef?: (el: HTMLDivElement | null) => void;
}

const CGStepCard = ({ step, cardRef }: CGStepCardProps) => {
  return (
    <div
      ref={cardRef}
      id={step.id}
      className="step-card bg-[#f0fbe3] relative mx-auto w-[320px] shrink-0 overflow-hidden rounded-4xl p-6 sm:w-[350px] md:h-[476px] md:w-[400px] xl:w-[442px]"
      data-step={step.stepNumber}>
      <figure className="bg-ns-green relative z-10 rounded-[20px] p-2">
        <Image src={step.mainImage} alt={`stap ${step.stepNumber}`} className="size-full object-cover" />

        {step.overlayImage && (
          <Image
            src={step.overlayImage}
            alt={`stap ${step.stepNumber}`}
            className={cn(
      ...
      <div
        ...
        style={{ background: 'linear-gradient(0deg, rgba(255, 255, 255, 0) 0%, #f0fbe3 37.44%)' }}
      />
